...
def build_match_firm_data(target_firm_data):
    ws = wb2.active
    matched_firms = {target_firm_data[i]["matched_firm"]:{"eventyear": target_firm_data[i]["eventyear"],
        "pre_eventyear": target_firm_data[i]["pre_eventyear"],
        "net_income_event_year_plus":{}} for i in target_firm_data if target_firm_data[i]["matched_firm"]}
    company_col = get_index('I')
    sic_col = get_index('BK')
    date_col = get_index('B')
    assets_col = get_index('M')
    ...
    for row in ws.iter_rows(row_offset=1):
        datecell = row[date_col].value
        firm = unicode(row[company_col].value).strip()
        if firm in matched_firms:
            sic_code = row[sic_col].value
            total_assets = row[assets_col].value
            net_income = row[net_income_col].value
            eventyear = matched_firms[firm]["eventyear"]
            pre_eventyear = matched_firms[firm]["pre_eventyear"]

            if datecell:
                date = int(str(datecell)[:4])
            if date == eventyear and total_assets:
                matched_firms[firm]["total_assets"] = total_assets
            if date >= eventyear and net_income:
                matched_firms[firm]["net_income_event_year_plus"][date] = net_income
            if date == pre_eventyear and net_income:
                matched_firms[firm]["net_income_pre_event_year"] = net_income
    for target_firm in target_firm_data:
        match = target_firm_data[target_firm]["matched_firm"]
        if match:
            try:
                target_firm_data[target_firm]["matched_firm_net_income_pre_event_year"] = matched_firms[match]["net_income_pre_event_year"]
            except KeyError as e:
                print "Key Error for: ", match , e
            try:
                target_firm_data[target_firm]["matched_firm_total_assets"] = matched_firms[match]["total_assets"]
            except KeyError as e:
                print "Key Error for: ", match , e
            try:
                target_firm_data[target_firm]["matched_firm_net_income_posteventyear"] = matched_firms[match]["net_income_event_year_plus"]
            except KeyError as e:
                print "Key Error for: ", match , e

    return target_firm_data



def create_match_output(output_file_name,entry_data,firm_type_header="Target Firm"):
    ...
    firms.sort()
    sample = firms[0]
    headers = [firm_type_header] + [record for record in entry_data[sample] if not hasattr(entry_data[sample][record],'__iter__')]
    ws.append(headers)
    for firm in firms:
...
def create_year_output(output_file_name,entry_data,record, year_start=1998,year_end=2014, firm_type_header="Target Firm"):
    output = os.path.join(output_path,output_file_name)
    col_end = year_end - year_start + 1
    wb = Workbook()
    ws=wb.active
    firms = [entry for entry in entry_data]
    matched_firms = [entry_data[firm]["matched_firm"] for firm in firms]
    firms.sort()
    matched_firms.sort()
    year_map = {i:j for i,j in zip(string.uppercase[1:col_end],range(year_start,year_end))}
    ws.cell(row=1,column=1).value = firm_type_header
    for row in range(2,len(firms) + 2):
        firm = firms[row-2]
        if firm_type_header == "Target Firm":
            ws.cell(row=row,column=1).value = firm
        if firm_type_header == "Matched Firm":
            ws.cell(row=row,column=1).value = entry_data[firm]["matched_firm"]
        for col in range(2,len(year_map) + 2):
            cell = ws.cell(row=row,column=col)
            year = year_map[cell.column]
...
if __name__ == "__main__":
    ...
    matched_firms = get_match(income_data,target_firms)
    complete_data = build_match_firm_data(matched_firms)
    create_match_output("target_firm_data.xlsx",complete_data)
    create_year_output("One_hundred_best_ranks.xlsx",complete_data,"100_best_ranks")
    create_year_output("target_firm_net_income_posteventyear.xlsx",complete_data,"net_income_event_year_plus")
    create_year_output("matched_firm_net_income_posteventyear.xlsx",complete_data,
                       "matched_firm_net_income_posteventyear", firm_type_header="Matched Firm")
